...
public class CountryCodesDBManager {
    private static final String TAG = CountryCodesDBManager.class.getSimpleName();
    private static final String DATA_PATH = Environment.getDataDirectory().getPath()
            + "/data/" + CommonValue.PACKAGE_NAME + "/databases/";
    ...
    private void copyDatabaseFile() {
        boolean isCreatedFolder = new File(DATA_PATH).mkdir();
        if (!isCreatedFolder) {
            Log.i(TAG, "Can not create new directory");
            return;
        }
        ...
        try {
            ...
            if (!isCreatedFile) {
                Log.i(TAG, "Can not create new file");
                return;
            }
    ...
    public ArrayList<CountryCodeItem> getData() {
        ...
        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM " + DATA_NAME, null);
        if (cursor == null)
            return null;
        cursor.moveToFirst();
