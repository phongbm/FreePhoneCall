...
public class FreePhoneCallService extends Service implements SinchClientListener {
    private static final String TAG = "FreePhoneCallService";
    ...
    private class InComingCallListener implements CallClientListener, CallListener {
        ...
        public void onIncomingCall(CallClient callClient, Call call) {
            ...
            intentInComingCall.setClassName(CommonValue.PACKAGE_NAME,
    ...
    private class MessageListener implements MessageClientListener {
        ...
        public void onIncomingMessage(final MessageClient messageClient, final Message message) {
            ...
            if (message.getHeaders().get("MAP") != null) {
                ...
                if (body.equals("MAP_OK")) {
                    ...
                    if (mapIntent.resolveActivity(FreePhoneCallService.this.getPackageManager()) != null) {
                        FreePhoneCallService.this.startActivity(mapIntent);
                    }
                    return;
                }
                final AlertDialog alertDialog = new AlertDialog.Builder(FreePhoneCallService.this).create();
                ...
                alertDialog.setButton(AlertDialog.BUTTON_POSITIVE, "YES",
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                if (ContextCompat.checkSelfPermission(FreePhoneCallService.this,
                                        Manifest.permission.ACCESS_FINE_LOCATION)
                                        != PackageManager.PERMISSION_GRANTED
                                        && ContextCompat.checkSelfPermission(FreePhoneCallService.this,
                ...
                return;*/
            /*}
            ActivityManager activityManager = (ActivityManager) FreePhoneCallService.this
            ...
            if (GlobalApplication.startActivityMessage) {
                ...
                FreePhoneCallService.this.sendBroadcast(intentSent);
            }
            ...
            if (tasks.get(0).processName.equals(CommonValue.PACKAGE_NAME_MAIN)) {
                ...
                if (!open) {
                    open = true;
                    FreePhoneCallService.this.openChatHead(message.getSenderId(),
                            message.getHeaders().get("senderName"), content, date);
                }
            }

            FreePhoneCallService.this.updateMessagesLogDBManager(id, fullName, content, date, isRead);
            if (!GlobalApplication.startWaitingAHihi) {
                ...
                FreePhoneCallService.this.sendBroadcast(intent);
            }
            if (GlobalApplication.checkLoginThisId) {
                ...
                FreePhoneCallService.this.sendBroadcast(intentIncoming);
            }*/
        }
        ...
        public void onMessageSent(MessageClient messageClient, final Message message, String s) {
            /*if (message.getHeaders().get("MAP") != null) {
                ...
                FreePhoneCallService.this.updateMessagesLogDBManager(id, fullName, "You:  " + content, date, 1);
                if (!GlobalApplication.startWaitingAHihi) {
                    GlobalApplication.startWaitingAHihi = true;
                    GlobalApplication.checkLoginThisId = false;
                    Intent intent = new Intent();
                ...
                if (GlobalApplication.checkLoginThisId) {
                    ...
                    FreePhoneCallService.this.sendBroadcast(intentMessage);
                }
            }*/
        }
        ...
        public void onMessageDelivered(MessageClient messageClient, MessageDeliveryInfo messageDeliveryInfo) {
            ...
            FreePhoneCallService.this.sendBroadcast(intentDelivered);*/
        }
            ...
            switch (intent.getAction()) {
                ...
                /*case CommonValue.ACTION_SEND_MESSAGE:
                    ...
                    if (intent.getStringExtra(CommonValue.AHIHI_KEY) == null) {
                        FreePhoneCallService.this.sendMessage(id, content, fullName, date);
                    } else {
                        switch (intent.getStringExtra(CommonValue.AHIHI_KEY)) {
                            case CommonValue.AHIHI_KEY_EMOTICON:
                                ...
                                break;
                            case CommonValue.AHIHI_KEY_FILE:
                                FreePhoneCallService.this.sendFile(id, content, fullName, date);
                                break;
                            case CommonValue.AHIHI_KEY_PICTURE:
                                FreePhoneCallService.this.sendPicture(id, content, fullName, date);
                                break;
                        }
                    }
                    break;
            ...
            public void run() {
                ...
                try {
                    ...
                    parseFile.saveInBackground(new SaveCallback() {
                        ...
                        public void done(ParseException e) {
                            if (e != null) {
                                Toast.makeText(FreePhoneCallService.this, "Missing send file",
                            ...
                            parseObject.saveInBackground(new SaveCallback() {
                                ...
                                public void done(ParseException e) {
                                    if (e != null) {
                                        ...
                                        Toast.makeText(FreePhoneCallService.this, "Error send file",
                                                Toast.LENGTH_SHORT).show();
                                        return;
                                    }
    ...
    /*private synchronized void sendPicture(final String id, final String pathPicture,
        ...
        if (!isChangeByte) {
            Toast.makeText(FreePhoneCallService.this, "Send image not success!!!", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        parseFile.saveInBackground(new SaveCallback() {
            ...
            public void done(ParseException e) {
                ...
                parseObject.saveInBackground(new SaveCallback() {
                    ...
                    public void done(ParseException e) {
                        if (e != null) {
                            Toast.makeText(FreePhoneCallService.this, "Send image not success!!!",
                                    Toast.LENGTH_SHORT).show();
                        ...
                        message.addHeader("fullName", fullName);
                        message.addHeader("senderName", senderName);
                        FreePhoneCallService.this.messageClient.send(message);
                    }
                });
            }
        }, new ProgressCallback() {
    ...
    /*private void openChatHead(final String id, final String fullName, final String content, final String date) {
        ...
        int index = ((GlobalApplication) FreePhoneCallService.this.getApplication()).getAllFriendItems()
                .indexOf(new AllFriendItem(id, 1));
        if (index > -1) {
            imgAvatar.setImageBitmap(((GlobalApplication) FreePhoneCallService.this
        ...
        widget.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intentChat = new Intent(FreePhoneCallService.this, MessageActivity.class);
                ...
                FreePhoneCallService.this.startActivity(intentChat);

                new Handler().post(new Runnable() {
                    @Override
                    public void run() {
                        FreePhoneCallService.this.updateMessagesLogDBManager(id, fullName, content, date, 1);
                        Intent intentIncoming = new Intent();
                        intentIncoming.setAction(CommonValue.UPDATE_MESSAGE_LOG);
                        intentIncoming.putExtra(CommonValue.MESSAGE_LOG_ID, id);
                        intentIncoming.putExtra(CommonValue.MESSAGE_LOG_FULL_NAME, fullName);
